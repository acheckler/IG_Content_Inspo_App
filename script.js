...
//Formatting user input for get request//
...
function getPictures(picSearchTerm, picColor) {
  ...
  fetch(url)
    ...
    .catch((err) => {
      $("#error-message").text(
        `Could not find a photo with given keyword(s). Please try using another search term.`
      );
    });
}
...
function getQuotes(quoteSearchTerm, quoteSearchType) {
  ...
  fetch(url)
    ...
    .catch((err) => {
      $("#error-message").text(
        `Could not find a quote with given keyword(s). Please try using another search term.`
      );
    });
}
...
//display pictures returned from unsplash//
